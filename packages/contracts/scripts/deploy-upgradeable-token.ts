...
async function main() {
  ...
  const FranekTataTokenUpgradeable = await hre.ethers.getContractFactory('FranekTataTokenUpgradeable');
  ...
  const token = await hre.upgrades.deployProxy(FranekTataTokenUpgradeable, [deployer.address], {
